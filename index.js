...
// Root route
app.get("/", (req, res) => {
  res.json({ message: "Books Database! Browse your favorite books" });
});
...
async function insertData(data) {
  try {
    const savedData = await Book.insertMany(data);
...
app.delete("/books/:id", async (req, res) => {
  try {
    ...
    res.json({ message: "Deleted successfully", deletedBook: deletedBook });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Internal server error", error: error.message });
